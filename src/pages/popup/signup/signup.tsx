...
const SignUp = () => {
    ...
    document.body.style.width = "522px";
    document.body.style.height = "600px";
    document.getElementsByTagName('html')[0].style.width = "522px"
    ...
    const [usageReason, setUsageReason] = useState<string>('lucy')
    const [howFind, setHowFind] = useState<string>('')
    ...
    const howfindRef = useRef(null)

    useEffect(() => {
        isSuccess && goTo(Popup)
    }, [isSuccess])
    ...
    const signup = async () => {
        if (email == '') {
            ...
            howfindRef.current.focus()
        } else if (password != confirmPassword) {
            messageApi.warning('Input the Password Correctly!')
            confirmpasswordRef.current.focus()
        } else {
            try {
                setloading(true)
                ...
                await messageApi.success('Success!')
                setSuccess(true)
            } catch (error) {
                messageApi.error('Failed SignUp!')
            }
        }
        setloading(false)
    }
    ...
    return (
        <div className="signup-wrapper">
            ...
            <div className="google-account-creation-wrapper">
                <h1>Create an account</h1>
                <Button className="google-auth-btn" onClick={handleGoogleSignIn}> <img src={GoogleIcon} /><span>Google</span></Button>
                <div className="divider-wrapper">
                    <div className="divider-line left-divider divider-width" />
                    <span>Or sign up with email</span>
                    <div className="divider-line right-divider divider-width" />
                </div>
            </div>
            <div className="account-creation-wrapper">
                <div className="inputs-wrapper">
                    <div className="input-wrapper">
                        ...
                        <Input id='email' className="font-Jakarta input-field" onChange={handleChange} ref={emailRef} placeholder="audrey@mapo.studio" />
                    </div>
                    <div className="input-wrapper">
                        <p className="input-label">Full Name</p>
                        <Input id='fullname' className="font-Jakarta input-field" onChange={handleChange} ref={fullnameRef} placeholder="Joe the Man" />
                    </div>
                </div>
                <div className="inputs-wrapper">
                    <div className="input-wrapper">
                        <p className="input-label">Password</p>
                        <Input.Password id='password' onChange={handleChange} ref={passwordRef} className="font-Jakarta input-field" />
                    </div>
                    <div className="input-wrapper">
                        <p className="input-label">Confirm Password</p>
                        <Input.Password id='confirmpassword' onChange={handleChange} ref={confirmpasswordRef} className="font-Jakarta input-field" />
                    </div>
                </div>
                <div className="inputs-wrapper">
                    <div className="input-wrapper">
                        <p className="input-label">Company</p>
                        <Input id='company' className="font-Jakarta input-field" onChange={handleChange} ref={companyRef} placeholder="Mapo Studio" />
                    </div>
                    <div className="input-wrapper">
                        <p className="input-label">Position</p>
                        <Input id='position' className="font-Jakarta input-field" onChange={handleChange} ref={positionRef} placeholder="Lead Designer" />
                    </div>
                </div>
                <div className="inputs-wrapper">
                    <div className="input-wrapper select-wrapper">
                        <p className="input-label">Why are you using Careerly?</p>
                        <Select
                            className="font-Jakarta"
                            style={{ width: '100%', fontFamily: 'PlusJakartaSans', height: '44px', padding: '0' }}
                            defaultValue='keeptrack'
                            onChange={handleSelect}
                            options={[
                                { value: 'keeptrack', label: 'I want to keep track of my reports' },
                                { value: 'bettermanager', label: 'I want to become a better manager' },
                                { value: 'companywants', label: 'My company expects me to use it' },
                                { value: 'trackprogress', label: 'I want to track my own progress/performance' }
                            ]}
                        />
                    </div>
                </div>
                <div className="inputs-wrapper">
                    <div className="input-wrapper select-wrapper">
                        <p className="input-label">How did you find out about us?</p>
                        <Input id='howfind' className="font-Jakarta input-field" onChange={handleChange} ref={howfindRef} placeholder="Mapo Studio" />
                    </div>
                </div>
            </div>
            <div className="loging-btn-wrapper">
                <Button className="login-btn remove-margin-top" loading={isLoading} type="primary" onClick={signup}>Sign Up</Button>
                <p className="signup-text">Already have an account? <span onClick={login}>Log In</span></p>
            </div>
